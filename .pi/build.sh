...
# docker buildx build --tag blog --file .pi/Dockerfile --platform linux/arm64 --cache-from blog:cache --cache-to blog:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar blog
...
sudo bash -c "echo blog > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/blog.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/blog.img
